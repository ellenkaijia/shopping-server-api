package com.manager.api.mybatis;
...
public class PageInterceptor implements Interceptor{
	...
	public Object intercept(Invocation invocation) throws Throwable{
        ...
        getSql(mappedStatement.getConfiguration(), boundSql, mappedStatement.getId(), time , this.showSql);
		return returnObj;
	}
	
	public static void getSql(Configuration configuration, BoundSql boundSql, String sqlId, double time , boolean isShowSql) {
		if(!isShowSql){
			return;
		}
		try{
			String classMethod = StringTools.getClassAndMethod(sqlId);
			//不输出SQL的类
			if(classMethod.contains("ConsoleFunctionDao.") || 
						classMethod.contains("CachedDao.") ){
				return;
			}
			String sql = showSql(configuration, boundSql);
	        StringBuilder str = new StringBuilder(100);
	        str.append(classMethod).
	            append(" : ").
	            append(time).
	            append("s\r\n").
	            append(StringTools.getStringOneBlank(sql.trim()));
	...
	private static String getParameterValue(Object obj) {
        ...
        if (obj instanceof String) {
        	...
        	value = DateUtils.toString((Date) obj, "yyMMddHHmmssSSS"); // "yyMMddHHmmssSSS";
        } else {
            if (obj != null) {
                value = obj.toString();
            } else {
	...
	/**
	 * 给当前的参数对象page设置总记录数
	 * @param pageMapper 映射语句对应的参数对象
	 * @param mappedStatementMapper 映射语句
	...
	private String getCountSql(String sql){
	    return "select count(*) from ( " + sql + " ) mysql_count_tmps";
	}
